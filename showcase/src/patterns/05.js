...
const ClapCount = ({ style: userStyles = {} }) => {
    ...
    return (
        <span ref={setRef} data-refkey='clapCountRef' className={styles.count} style={userStyles}>
            +{count}
        </span>
    )
}
const CountTotal = ({ style: userStyles = {} }) => {
    ...
    return (
        <span ref={setRef} data-refkey='clapTotalRef' className={styles.total} style={userStyles}>
            {countTotal}
        </span>
    )
}
...
const Usage = () => {
    ...
    return (     
        <div style={{ width: '100%'}}>
            <MediumClap onClap={handleClap} style={{ border: '1px solid #8cacea'}}>
                <MediumClap.Icon style={{ fill: '#8cacea'}} />
                <MediumClap.Count style={{ background: '#8cacea'}} />
                <MediumClap.Total style={{ background: '#8cacea', color: 'black' }} />
            </MediumClap>
            { count !== 0 && <div className={styles.info} >{`You have clapped ${count} times`}</div>}
        </div>
    )   
}
